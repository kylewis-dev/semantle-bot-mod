...
def scale_similarity(similarity_score):
    return int(similarity_score * 1000)
...
def find_possible_vectorized(guess, reported_sim, words_to_consider):
    ...
    candidates = np.where(np.abs(cosines - reported_sim) <= 5)[0]
...
def find_possible(guess, reported_sim, words_to_consider):
    ...
    for w in tqdm(words_to_consider):
        if abs(scale_similarity(similarity(guess, w)) - reported_sim) <= 5:
            ans.append(w)
    return ans
