...
def find_possible_vectorized(guess, reported_sim, words_to_consider):
    ...
    cosines = (numerator / denominator) * 1000  # num_words x 1, cosines[i] = SEMANTLE cosine of guess with ith wv
...
def similarity(word1, word2):
    return int((1 - spatial.distance.cosine(wv[word1], wv[word2])) * 1000)
...
def find_possible(guess, reported_sim, words_to_consider):
    ...
    for w in tqdm(words_to_consider):
        if abs(similarity(guess, w) - reported_sim) <= 1:
            ans.append(w)
    return ans
